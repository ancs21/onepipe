...
```mermaid
flowchart LR
    A[Your App] -->|append| F[Flow]
    F -->|store| DB[(PostgreSQL)]
    F -->|notify| S1[Subscriber 1]
    F -->|notify| S2[Subscriber 2]
```
...
Events stored in `_onepipe_flow_events`:
...
All instances see all events. Survives restarts.
...
sequenceDiagram
    ...
    App->>Flow: append(event)
    Flow->>Flow: validate schema
    alt validation fails
        Flow-->>App: throw error
    end
    Flow->>DB: INSERT
    Flow->>Subs: notify all
    Flow-->>App: return event with ID
```
...
Flows pair naturally with Projections:

```mermaid
flowchart LR
    FLOW[Flow] -->|events| PROJ[Projection]
    PROJ -->|reduce| STATE[Computed State]
```
