...
sequenceDiagram
    ...
    App->>WF: start(order)
    WF->>DB: save workflow (status: running)

    WF->>WF: step('validate')
    WF->>DB: save step result
    ...
    App->>WF: recover()
    WF->>DB: load workflow + steps
    WF->>WF: skip 'validate' (done)
    WF->>WF: skip 'charge' (done)
    WF->>WF: step('confirm') ← resume
    WF->>DB: save step result
```
...
erDiagram
    ...
    _onepipe_workflows ||--o{ _onepipe_workflow_steps : has
    _onepipe_workflows ||--o{ _onepipe_workflow_signals : receives
```
...
```mermaid
flowchart TD
    A[ctx.step called] --> B{Already in DB?}
...
sequenceDiagram
    ...
    WF->>DB: waiting for signal 'approval'

    loop Poll every 1s
        WF->>DB: check for signal
    end

    API->>DB: insert signal value
    DB-->>WF: signal found
...
Long-running workflows update `updated_at` every 10 seconds. This lets you detect stuck workflows:

```mermaid
flowchart LR
    WF[Workflow] -->|every 10s| DB[(updated_at = NOW)]
```

If `updated_at` is older than 30 seconds, the workflow probably crashed. `recover()` picks these up.
